...
const Inventory: React.FC<InventoryProps> = ({ state, updateState }) => {
  ...
  const handleScan = (barcode: string) => {
    ...
    if (!existing) {
      const wantToRegister = confirm(`Product with barcode ${barcode} not found. Register now?`);
      if (wantToRegister) {
        setEditingProduct({
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">View and manage your product stock.</p>
        </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Barcode</th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Stock</th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Cost</th>
              ...
              {filteredProducts.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-gray-500">
                    No products found. Add one or try a different search.
                  </td>
                </tr>
              )}
            </tbody>
            ...
            <form onSubmit={handleSaveProduct} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input name="category" defaultValue={editingProduct?.category} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. Dairy" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input name="unit" defaultValue={editingProduct?.unit} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="kg, pc, pack" />
                </div>
              </div>
              <div className="pt-4 flex justify-end gap-3">
                <button type="button" onClick={() => { setIsAdding(false); setEditingProduct(null); }} className="px-6 py-2 border rounded-lg hover:bg-gray-50 transition-colors">Cancel</button>
                <button type="submit" className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Save Product</button>
              </div>
            </form>
          </div>
        </div>
